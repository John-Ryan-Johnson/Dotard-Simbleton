...
const addressStateCode = "addressStateCode";
...
const manufacturingoutEl = document.querySelector("#manuBusi");
manufacturingoutEl.innerHTML = "<h1>Manufacturing Businesses</h1>";

manufacturingBusinesses.forEach(business => {
  manufacturingoutEl.innerHTML += `
      <h2>${business.companyName}</h2>
      <section>
        ${business.addressFullStreet}
      </section>
    ...
    `;
  manufacturingoutEl.innerHTML += "<hr/>";
});
...
const purchasingAgentOutEl = document.querySelector("#purchaseAgent");
purchasingAgentOutEl.innerHTML = "<h1>Purchasing Agents</h1>";
const agents = businesses.map(business => {
  const newObj = {};
  newObj.fullName = `${business.purchasingAgent.nameFirst} ${
    business.purchasingAgent.nameLast
  }`;
  newObj.company = business.companyName;
  newObj.phoneNumber = business.phoneWork;
  return newObj;
});

agents.forEach(agent => {
  purchasingAgentOutEl.innerHTML += `<h2>${agent.fullName}</h2>
  <h3>${agent.company}</h3>
  <p>${agent.phoneNumber}</p>`;
  purchasingAgentOutEl.innerHTML += "<hr/>";
});
...
const totalRainfall = monthlyRainfall.reduce((total, monthly) => total + monthly);
...
const sentence = words.reduce((words, sentence) => words + sentence);
